...
interface Props {
  ...
  modelValue?: any;
  error?: string;
  options: { text: string; key: string | number }[];
}

const { id, name, placeholder, required, modelValue, error, options } =
  defineProps<Props>();
</script>
    ...
    >
      <div>
        <select
          ...
          "
          class="form-select dark:bg-slate-900 text-sm text-slate-500 dark:text-slate-300 border border-slate-300 dark:border-slate-800 rounded focus:outline-none focus:border-sky-500 dark:focus:border-slate-500 focus:ring focus:ring-sky-100 dark:focus:ring-slate-600 w-full py-1.5 px-2.5"
          :class="[
            error
